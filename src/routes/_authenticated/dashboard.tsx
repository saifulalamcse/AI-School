...
function CoursesTab({ enrollments }: { enrollments: Enrollment[] }) {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-3">
      {enrollments.map((e) => (
        <div key={e.id} className="surface-card p-6 hover:-translate-y-1 transition">
          ...
          <div className="mt-4 h-1.5 rounded-full bg-white/10 overflow-hidden">
            <div className="h-full w-1/4 gradient-bg" />
          </div>
          <p className="mt-2 text-xs text-muted-foreground">25% complete</p>
          <Link
            to="/courses/$slug"
            params={{ slug: e.course_slug }}
            className="mt-5 inline-block text-sm gradient-text font-medium"
          >
            Continue learning →
          </Link>
        </div>
